...
// ATIVAÇÕES
async function listarAparelhosPrice({
    anoMes,
    filial,
    grupo_economico
  }) {
    return new Promise(async (resolve, reject) => {
      if(!grupo_economico){
        reject('[APARELHOS_PRICE]: Grupo não informado!')
        return false;
      }
      ...
      try {
        ...
        if (filial) {
          [ativacoes] = await db.execute(
            `SELECT id, gsm, gsmProvisorio, 
            CASE WHEN status_ativacao != 'Ativo' AND status_ativacao != 'Analise Pendente' THEN 'Inativo' 
            ELSE status_ativacao
            END as status_ativacao,
             modalidade, plaOpera, plano_ativado, dtAtivacao, cpf_cliente, cliente, vendedor, filial, imei, aparelho, fidAparelho, fidPlano, status_fid_aparelho, status_fid_plano, pedido, statusLinha, obs_ativ, obs_ativ_adm  
            FROM ${facell_docs} 
            WHERE 
            grupo_economico = ? 
            and filial = ?
