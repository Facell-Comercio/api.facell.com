const { format, startOfDay, formatDate, addDays, isMonday, isThursday, isSaturday, isSunday, subDays } = require("date-fns");
...
function insertOneByGN(req) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const url_contrato = process.env.BASE_URL + '/uploads/lacamento-automatico.pdf' //^ fixar um anexo salvo em uploads
      const id_tipo_solicitacao = 3 // 3 - Sem nota fiscal
      const id_status = 3
      const id_forma_pagamento = 1

      // Buscar o Centro de custo pelo grupo econômico
      const [rowCentroCusto] = await conn.execute(`SELECT id FROM fin_centros_custo WHERE nome = 'COMPRAS' AND id_grupo_economico = ?`, [id_grupo_economico])
      const id_centro_custo = rowCentroCusto && rowCentroCusto[0] && rowCentroCusto[0]['id']
      if(!id_centro_custo){
        throw new Error(`Centro de custo COMPRAS não localizado para o id_grupo_economico ${id_grupo_economico}. Providencie o cadastro junto à Administração.`)
      }
      ...
      const [rowPlanoConta] = await conn.execute(`SELECT id FROM fin_plano_contas WHERE nome = 'COMPRA DE MERCADORIA PARA REVENDA - APARELHO' AND id_grupo_economico = ?`, [id_grupo_economico])

      const id_plano_conta = rowPlanoConta && rowPlanoConta[0] && rowPlanoConta[0]['id']
      if(!id_plano_conta){
        throw new Error(`Plano de contas COMPRA DE MERCADORIA PARA REVENDA - APARELHO não localizado para o id_grupo_economico ${id_grupo_economico}. Providencie o cadastro junto à Administração.`)
      }
      ...
      const [insertedTitulo] = await conn.execute(`INSERT INTO fin_cp_titulos
      (
        id_fornecedor,
        id_filial,
        id_grupo_economico,
      ...
      //* Salvar o vencimento
      await conn.execute(`INSERT INTO fin_cp_titulos_vencimentos (id_titulo, data_vencimento, data_prevista, valor) VALUES (?,?,?,?)`,
      [
        id_titulo, 
