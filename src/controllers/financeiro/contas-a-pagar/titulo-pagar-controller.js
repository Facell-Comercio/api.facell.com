...
function getAllRecorrencias(req) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (!checkUserPermission(req, 'MASTER') && !checkUserDepartment(req, 'FINANCEIRO')) {
        const centro_custo = user?.centros_custo?.map(centro => centro.id).join(',');
        where += ` AND r.id_centro_custo IN(${centro_custo})`
      }
      const [recorrencias] = await db.execute(`SELECT r.* FROM fin_cp_titulos_recorrencias r ${where}`, params);
...
function insertOne(req) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      // Obter o Orçamento:
      const [rowOrcamento] = await conn.execute(`SELECT id FROM fin_orcamento WHERE DATE_FORMAT(ref, '%Y-%m') = ? and id_grupo_economico = ?`, [format(new Date(), 'yyyy-MM'), id_grupo_economico])
      ...
      for (const item of itens) {
        ...
        const [rowOrcamentoConta] = await conn.execute(`SELECT id, valor_previsto FROM fin_orcamento_contas 
          WHERE 
            id_orcamento = ?
            AND id_centro_custo = ?
            AND id_plano_contas = ?
        ...
        if (!rowOrcamentoConta || rowOrcamentoConta.length === 0) {
          throw new Error(`Não existe conta no orçamento para o seu Centro de custos + Plano de contas ${item.plano_conta}!`)
        }
        const id_orcamento_conta = rowOrcamentoConta && rowOrcamentoConta[0] && rowOrcamentoConta[0]['id']
        let valor_previsto = rowOrcamentoConta && rowOrcamentoConta[0] && rowOrcamentoConta[0]['valor_previsto']
        valor_previsto = parseFloat(valor_previsto);

        // Obter o Valor Realizado da Conta do Orçamento:
        const [rowConsumoOrcamento] = await conn.execute(`SELECT sum(valor) as valor 
          FROM fin_orcamento_consumo 
          WHERE active = true AND id_orcamento_conta = ?`, [id_orcamento_conta])
        let valor_total_consumo = rowConsumoOrcamento && rowConsumoOrcamento[0] && rowConsumoOrcamento['valor'] || 0;
        ...
        if (saldo < item.valor) {
          throw new Error(`Saldo insuficiente para o seu Centro de Custos + Plano de contas: ${item.plano_conta}. Necessário ${normalizeCurrency(item.valor - saldo)}`)
        }
      }

      // Persitir os anexos
      const nova_url_nota_fiscal = await moverArquivoTempParaUploads(url_nota_fiscal);
      ...
      const [resultInsertTitulo] = await conn.execute(`INSERT INTO fin_cp_titulos 
        (
          id_solicitante,
          id_fornecedor,
          id_banco,
      ...
      for (const item_rateio of itens_rateio) {
        await conn.execute(`INSERT INTO fin_cp_titulos_rateio (id_titulo, id_rateio, id_filial, percentual) VALUES (?,?,?,?)`,
      ...
      for (const item of itens) {
        ...
        const [resultNewItem] = await conn.execute(`INSERT INTO fin_cp_titulos_itens (id_titulo, id_plano_conta, valor) VALUES (?,?,?)`, [newId, item.id_plano_conta, item.valor])
        ...
        await conn.execute(`INSERT INTO fin_orcamento_consumo (id_orcamento_conta, id_titulo_item, valor) VALUES (?,?,?)`, [id_orcamento_conta, resultNewItem.insertId, item.valor])
      }

      //~ Fim de manipulação de itens //////////////////////

      // Persistir os itens do rateio
      const [itens_no_banco] = await conn.execute(`SELECT * FROM fin_cp_titulos_itens WHERE id_titulo = ?`, [newId])
      for (const item of itens_no_banco) {
        ...
        for (const item_rateio of itens_rateio) {
          ...
          await conn.execute(`INSERT INTO fin_cp_titulos_rateio_itens (id_titulo, id_titulo_item, id_rateio, id_filial, percentual, valor) VALUES (?,?,?,?,?,?)`,
            [newId, item.id, id_rateio, item_rateio.id_filial, item_rateio.percentual, valor_rateado])
        }
      }
      ...
      await conn.execute(`INSERT INTO fin_cp_titulos_historico (id_titulo, descricao) VALUES (?,?)`, [newId, historico])
...
function insertOneRecorrencia(req) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const [rowsExistentes] = await conn.execute(`SELECT id FROM fin_cp_titulos_recorrencias WHERE id_titulo = ?`, [id])
      ...
      const [resultInsertRecorrencia] = await conn.execute(`INSERT INTO fin_cp_titulos_recorrencias 
        (
          id_criador,
          id_titulo,
          id_fornecedor,
      ...
      for (const item of itens) {
        await conn.execute(`INSERT INTO fin_cp_titulos_recorrencias_itens (id_recorrencia, id_plano_conta, valor) VALUES (?,?,?)`, [newId, item.id_plano_conta, item.valor])
      }
      //~ Fim de manipulação de itens //////////////////////
...
function update(req) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const [itens_anteriores] = await conn.execute(`SELECT ti.valor, CONCAT(pc.codigo, ' - ', pc.descricao) as plano_conta
        FROM fin_cp_titulos_itens ti
        INNER JOIN fin_plano_contas pc ON pc.id = ti.id_plano_conta
        WHERE ti.id_titulo = ?`, [titulo.id])
      ...
      if (update_rateio || update_itens) {
        ...
        await conn.execute(`DELETE FROM fin_cp_titulos_rateio_itens WHERE id_titulo = ?`, [id])
      ...
      if (update_itens) {
        ...
        for (const item of itens) {
          ...
          const [rowConsumoTitulo] = await conn.execute(`SELECT sum(foc.valor) as valor 
          ...
          let valorConsumidoPeloItemAnterior = rowConsumoTitulo && rowConsumoTitulo[0] && rowConsumoTitulo[0]['valor'] || 0;
          valorConsumidoPeloItemAnterior = parseFloat(valorConsumidoPeloItemAnterior);
          ...
          // Calcular o saldo da conta do orçamento:
          const saldo = valor_previsto - valor_total_consumo + valorConsumidoPeloItemAnterior;
          if (saldo < item.valor) {
            throw new Error(`Saldo insuficiente para o seu Centro de Custos + Plano de contas: ${item.plano_conta}. Necessário ${normalizeCurrency(item.valor - saldo)}`)
          }
        }
        ...
        await conn.execute(`DELETE FROM fin_cp_titulos_itens WHERE id_titulo = ?`, [id])

        // ! Excluir todas as contas de consumo do orçamento anteriores
        await conn.execute(`DELETE FROM fin_orcamento_consumo 
        ...
        for (const item of itens) {
          // * Persistir o item do titulo e obter o id:
          const [result] = await conn.execute(`INSERT INTO fin_cp_titulos_itens (id_titulo, id_plano_conta, valor) VALUES (?,?,?)`, [id, item.id_plano_conta, item.valor])
          ...
          // * Persistir a conta de consumo do orçamento:
          await conn.execute(`INSERT INTO fin_orcamento_consumo (id_orcamento_conta, id_titulo_item, valor) VALUES (?,?,?)`, [id_orcamento_conta, result.insertId, item.valor])
        }
      }
      //~ Fim de manipulação de itens //////////////////////
      ...
      if (update_itens || update_rateio) {
        // Persistir os itens do rateio
        const [itens_no_banco] = await conn.execute(`SELECT * FROM fin_cp_titulos_itens WHERE id_titulo = ?`, [id])
        for (const item of itens_no_banco) {
          // * Persistir o rateio dos itens
          for (const item_rateio of itens_rateio) {
            const valor_rateado = item_rateio.percentual * item.valor;
            console.log(item_rateio.percentual, item.valor, valor_rateado)
            await conn.execute(`INSERT INTO fin_cp_titulos_rateio_itens (id_titulo, id_titulo_item, id_rateio, id_filial, percentual, valor) VALUES (?,?,?,?,?,?)`,
              [id, item.id, id_rateio, item_rateio.id_filial, item_rateio.percentual, valor_rateado])
          }
        }
      }
      ...
      await conn.execute(`UPDATE fin_cp_titulos 
      SET
        id_fornecedor = ?,
        id_banco = ?,
        id_forma_pagamento = ?,
      ...
      // Gerar e Registar historico:
      let historico =
        `EDITADO POR: ${normalizeFirstAndLastName(user.nome)}.\n`
      if (data_vencimento != titulo.data_vencimento) {
        historico += `VENCIMENTO: DE: ${format(titulo.data_vencimento, 'dd/MM/yyyy')} PARA: ${format(data_vencimento, 'dd/MM/yyyy')}\n`
      }
      if (valor != titulo.valor) {
        historico += `VALOR: DE: ${normalizeCurrency(titulo.valor)} PARA: ${normalizeCurrency(valor)}\n`
      }
      ...
      if (update_itens) {
        ...
        itens_anteriores.forEach((item_anterior, index) => {
          ...
          historico += `\t VALOR: '${normalizeCurrency(item_anterior.valor)}' \n`
        })
      }

      await conn.execute(`INSERT INTO fin_cp_titulos_historico (id_titulo, descricao) VALUES (?,?)`, [id, historico])
