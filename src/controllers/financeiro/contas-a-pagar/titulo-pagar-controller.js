...
function importSolicitacaoLote(req) {
  return new Promise(async (resolve, reject) => {
      ...
      for (const titulo of titulos) {
        ...
        try {
          if (id_status !== 3 && format(data_vencimento, "yyyyMMdd") < today) {
            throw new Error(
              `Data de vencimento da solicitação inferior a data atual`
            );
          }
          ...
          const cod_barras = !!codigo_barras
            ? normalizeCodigoBarras(codigo_barras)
            : null;
          if (!!cod_barras && !checkCodigoBarras(cod_barras)) {
            throw new Error(`Linha Digitável inválida: ${cod_barras}`);
          }
          ...
          await conn.execute(
            ...
            [
              idTitulo,
              `CRIADO POR: ${normalizeFirstAndLastName(
                req.user.nome
              )}. LANÇAMENTO EM LOTE\n`,
            ]
          );
          if (id_status === 3) {
            await conn.execute(
              ...
              [
                idTitulo,
                `APROVADO POR: ${normalizeFirstAndLastName(
                  req.user.nome
                )}. LANÇAMENTO EM LOTE\n`,
              ]
            );
