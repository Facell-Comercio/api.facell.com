...
function getAll(req) {
  return new Promise(async (resolve, reject) => {
    ...
    if (tipo_data && range_data) {
      ...
      const campo_data = tipo_data == 'data_prevista' || tipo_data == 'data_vencimento' ? `tv.${tipo_data}` : `t.${tipo_data}`
...
function update(req) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const [vencimentosPagos] = await conn.execute('SELECT id FROM fin_cp_titulos_vencimentos WHERE id_titulo = ? AND NOT data_pagamento IS NULL', [id])
      if (vencimentosPagos && vencimentosPagos.length) {
        throw new Error(`Impossível editar a solicitação pois já existem ${vencimentosPagos.length} vencimentos pagos..`)
      }
      ...
      if (vencimentosEmBordero && vencimentosEmBordero.length > 0) {
        throw new Error(`Você não pode alterar a solicitação pois ${vencimentosEmBordero.length} vencimentos já estão em bordero de pagamento.`)
      }
      ...
      if (update_vencimentos) {
        ...
        vencimentos_anteriores.forEach((venc_anterior, index) => {
          ...
          historico += `\t DATA VENC.: '${formatDate(venc_anterior.data_vencimento, 'dd/MM/yyyy')}' \n`;
          historico += `\t DATA PREV..: '${formatDate(venc_anterior.data_prevista, 'dd/MM/yyyy')}' \n`;
...
function changeStatusTitulo(req) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const [vencimentosPagos] = await conn.execute(`SELECT id FROM fin_cp_titulos_vencimentos WHERE id_titulo = ? AND NOT data_pagamento IS NULL`, [id_titulo])
