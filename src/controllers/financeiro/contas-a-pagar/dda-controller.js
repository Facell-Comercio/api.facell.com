...
const { normalizeCodigoBarras } = require("../remessa/to-string/masks");
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            if (tipo_data && range_data) {
                ...
                if (data_de && data_ate) {
                    where += ` AND dda.${tipo_data} BETWEEN '${data_de.split("T")[0]}' AND '${data_ate.split("T")[0]
                        }'  `;
                } else {
                    if (data_de) {
...
async function vincularDDA(req) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            const [rowVencimentosVinculados] = await conn.execute(`SELECT id, cod_barras FROM fin_dda WHERE id_vencimento = ?`, [id_vencimento])
            if(rowVencimentosVinculados && rowVencimentosVinculados.length > 0){
                throw new Error(`Vencimento de ID ${id_vencimento} já foi vinculado com o DDA ID: ${rowVencimentosVinculados[0]['id']}, código de barras: ${rowVencimentosVinculados[0]['cod_barras']}!`)
            }
