...
function pagarVencimento({ user, conn, vencimento, data_pagamento }) {
    return new Promise(async (resolve, reject) => {
        try {
...
function pagarFatura({ user, conn, fatura, data_pagamento }) {
    return new Promise(async (resolve, reject) => {
        try {
...
module.exports = function pagamentoItens(req) {
    return new Promise(async (resolve, reject) => {

        let conn
        try {
            const user = req.user;
            const { id_bordero, itens, data_pagamento } = req.body;
            ...
            await Promise.all(vencimentos.map(vencimento => pagarVencimento({ user, conn, vencimento, data_pagamento })))

            // * PAGAMENTO DAS FATURAS
            await Promise.all(faturas.map(fatura => pagarFatura({ user, conn, fatura, data_pagamento })))
