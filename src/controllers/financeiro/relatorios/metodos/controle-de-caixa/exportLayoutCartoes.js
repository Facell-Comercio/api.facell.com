  ...
  try {
    ...
    for (const filial of filiais) {
      ...
      const [caixas] = await conn.execute(
        `
          SELECT data, valor_cartao, valor_cartao_real FROM datasys_caixas
          ${where} AND id_filial = ?
          `,
        [...params, filial.id]
      );
      ...
      for (const caixa of caixas) {
        const obj = {
          Filial: filial.nome,
          Data: caixa.data,
          Datasys: parseFloat(caixa.valor_cartao || "0"),
          Adquirente: parseFloat(caixa.valor_cartao_real || "0"),
          "Diferença": parseFloat(caixa.valor_cartao - caixa.valor_cartao_real || "0"),
        };
    ...
    const filename = `RELATORIO CARTOES DATASYS X ADQUIRENTE (${mes}-${ano}) ${formatDate(
      new Date(),
      "dd-MM-yyyy hh.mm"
    )}.xlsx`;
    ...
    logger.error({
      module: "FINANCEIRO",
      origin: "RELATORIOS",
      method: "EXPORT_RELATORIO_CARTAO",
