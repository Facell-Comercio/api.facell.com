...
const supervisorRelacionamento = async ({ ref, agregador, politica }) => {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      var listaMetas = agregador.metas_agregadas.split(";").map((cpf) => cpf.trim());

      const filterMeta = ` AND cpf in(${listaMetas.map((value) => db.escape(value)).join(",")})`;
    ...
    `,
      ...
      const [rowsInadimplencias] = await db.execute(
        ...
        WHERE 
            ...
            AND cpfVendedor in(${listaMetas.map((value) => db.escape(value)).join(",")})
    `,
        [refInadimplencia]
      );
      ...
      const [realizadoServico] = await db.execute(
        ...
        WHERE
            ...
            AND v.cpfVendedor in(${listaMetas.map((value) => db.escape(value)).join(",")})
      ...
      espelho.menor_ating_tim = Math.min(ating_receita);
      ...
      let ating_aparelho_vendas_indiretas =
        espelho.metas.aparelho_vendas_indiretas == 0
          ? 1
          : parseFloat(
              (real_aparelho_vendas_indiretas / espelho.metas.aparelho_vendas_indiretas).toFixed(4)
            );
      ...
      const bonus_a_calcular = [
        ...
        },
        {
          ...
          atingimento: Math.min(espelho.atingimento.aparelho, espelho.atingimento.acessorio),
          escalonamento: Math.min(espelho.escalonamento.aparelho, espelho.escalonamento.acessorio),
        ...
        },
        {
          ...
          atingimento: Math.min(espelho.atingimento.receita, espelho.atingimento.pitzi),
          escalonamento: Math.min(espelho.escalonamento.receita, espelho.escalonamento.pitzi),
        ...
        //   Esses são peculiares:
        {
          ...
          atingimento: Math.min(espelho.menor_ating_tim, espelho.menor_ating_facell),
          escalonamento: verificaEscalonamento(
            Math.min(espelho.menor_ating_tim, espelho.menor_ating_facell)
          ),
          faturamento:
            espelho.realizado.aparelho +
            espelho.realizado.aparelho_vendas_indiretas +
