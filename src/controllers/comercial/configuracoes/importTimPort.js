...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // const excelFileList = importFromExcel(filePath, { sheetName: "CV_PÓS TOTAL" });
    ...
    for (const pessoa of excelFileList) {
      ...
      try {
        ...
        arrayPessoas.push(`(
          ...
          ${db.escape(pessoa["PORT"])}
          )
        `);

        if (arrayPessoas.length === maxLength || totalPessoas === 1) {
          const queryInsert = `
            INSERT IGNORE INTO tim_portabilidade
            (
              ref,
              id_filial,
              cpf,
    ...
    if (excelFileList.length > 0) {
      await conn.execute(
        ...
        [user.id, "IMPORT_TIM_PORTABILIDADE", `Foram importados ${excelFileList.length} registros!`]
      );
    }
    ...
    logger.error({
      ...
      method: "IMPORT_TIM_PORTABILIDADE",
