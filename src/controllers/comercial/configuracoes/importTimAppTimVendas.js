...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const excelFileList = importFromExcel(filePath, { options: { range: 1 } }).filter(
      (pessoa) =>
        pessoa["NICKNAME"] !== "Total" &&
        pessoa["NOME_FUNC"] !== "Total" &&
        pessoa["NOME_FUNC"] !== "" &&
        pessoa["NOME_FUNC"] !== null
    );
    ...
    for (const pessoa of excelFileList) {
      ...
      try {
        const [rowPessoa] = await conn.execute(
          ...
          `,
          [String(pessoa["NOME_FUNC"]).split("-")[0].trim(), String(pessoa["MATRICULA"])]
        );
        ...
        obj["TOTAL"] = parseInt(pessoa["SIEBEL"]) + parseInt(pessoa["APP"]);

        arrayPessoas.push(`(
          ${db.escape(new Date(ano, mes - 1, 1))},
          ${db.escape(pessoaBanco.id_filial)},
          ${db.escape(pessoaBanco.cpf)},
          ${db.escape(parseInt(pessoa["SIEBEL"]) + parseInt(pessoa["APP"]))},
          ${db.escape(pessoa["APP"])}
          )
        `);

        if (arrayPessoas.length === maxLength || totalPessoas === 1) {
          const queryInsert = `
            INSERT IGNORE INTO tim_app_x_siebel
            (
              ref,
              id_filial,
              cpf,
    ...
    logger.error({
      ...
      method: "IMPORT_TIM_APP_TIM_VENDAS",
