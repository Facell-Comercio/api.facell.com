...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const excelFileList = importFromExcel(filePath, { options: { range: 1 } }).filter(
      (pessoa) =>
        pessoa["NOME_VENDEDOR"] !== "Total" &&
        pessoa["NOME_VENDEDOR"] !== "" &&
        pessoa["NOME_VENDEDOR"] !== null
    );
    ...
    for (const pessoa of excelFileList) {
      ...
      try {
        ...
        obj["TOTAL"] = parseInt(pessoa["App"]) + parseInt(pessoa["Total"]);
        ...
        if (arrayPessoas.length === maxLength || totalPessoas === 1) {
          const queryInsert = `
            INSERT IGNORE INTO tim_esteira_full
            (
              ref,
              id_filial,
              cpf,
    ...
    if (excelFileList.length > 0) {
      await conn.execute(
        ...
        [user.id, "IMPORT_TIM_ESTEIRA_FULL", `Foram importados ${excelFileList.length} registros!`]
      );
    }
    ...
    logger.error({
      ...
      method: "IMPORT_TIM_ESTEIRA_FULL",
